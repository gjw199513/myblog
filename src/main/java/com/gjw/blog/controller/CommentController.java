...
public class CommentController {
    ...
    public String listComments(@RequestParam(value = "blogId", required = true) Long blogId, Model model) {
        ...
        if (SecurityContextHolder.getContext().getAuthentication() != null && SecurityContextHolder.getContext().getAuthentication().isAuthenticated()
            ...
            if (principal != null) {
                commnetOwner = principal.getUsername();
            }
        }

        model.addAttribute("commentOwner", commnetOwner);
     ...
     *
     * @param blogId
     * @param commnetContent
     * @return
     */
    @PostMapping
    @PreAuthorize("hasAnyAuthority('ROLE_ADMIN','ROLE_USER')") // 指定角色权限才能操作方法
    public ResponseEntity<Response> createComment(Long blogId, String commnetContent) {
        try {
            blogService.createComment(blogId, commnetContent);
